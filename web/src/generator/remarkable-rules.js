...
export default function customRemarkable(md, pluginOptions) {
  ...
  md.renderer.rules.image = ((pluginOptions) => (tokens, idx, options, env) => {
    const src = ` src="${pluginOptions.pathname ? `${escapeHtml(pluginOptions.pathname)}/` : ''}${escapeHtml(
      tokens[idx].src
    )}"`;
    ...
    const classes = ` class="${pluginOptions.classes ? escapeHtml(replaceEntities(pluginOptions.classes)) : ''}"`;
